...
Сервис запускается из коробки через `docker-compose up` с дефолтными значениями
переменных окружения.

### База данных (Postgres)

Эти значения задаются в `docker-compose.yml`:

| Переменная         | Значение по умолчанию | Где используется          |
|--------------------|-----------------------|---------------------------|
| `POSTGRES_USER`    | `postgres`            | сервис `pr-db`            |
| `POSTGRES_PASSWORD`| `password`            | сервис `pr-db`            |
| `POSTGRES_DB`      | `postgres`            | сервис `pr-db`            |
...
| Переменная           | Значение по умолчанию                                               | Где используется                                     |
|----------------------|----------------------------------------------------------------------|------------------------------------------------------|
| `TEST_DATABASE_URL`  | `postgres://postgres:password@localhost:5432/postgres?sslmode=disable` | `make test-integration` (см. `Makefile`)            |

### Нагрузочное тестирование

Скрипт `loadtest/k6_pr_service.js` использует переменную:

| Переменная | Значение по умолчанию     | Где используется                                 |
|-----------|----------------------------|--------------------------------------------------|
| `BASE_URL`| `http://localhost:8080`    | нагрузочный тест (`loadtest/k6_pr_service.js`)   |

Все указанные значения подходят для локального запуска и проверки задания.
Изменять их не обязательно.
